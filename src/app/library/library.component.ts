...
export class LibraryComponent implements OnInit, OnDestroy {
  ...
  async getLibraryResource(type: string, offset?: number) {
    ...
    switch (type) {
      case 'recently-added': {
        results = await this.musicService.musicKit.api.library.collection('recently-added', null, { offset: offset, limit: 10 });
      ...
      case 'artists': {
        results = await this.musicService.musicKit.api.library.artists({ offset: offset, limit: 100 });
      ...
      case 'albums': {
        results = await this.musicService.musicKit.api.library.albums(null, { offset: offset });
      ...
      case 'songs': {
        results = await this.musicService.musicKit.api.library.songs(null, { offset: offset });
      ...
      case 'playlists': {
        results = await this.musicService.musicKit.api.library.playlists(null, { offset: offset });
